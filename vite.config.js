...
const pageRoutes=server=>{server.middlewares.use((req,res,next)=>{for(const path of ['/welcome','/project'])if(req.url===path||req.url?.startsWith(`${path}?`))req.url=req.url.replace(path,`${path}.html`);next();});};
...
return {
  ...
  build: { target: 'es2022', chunkSizeWarningLimit: 900, rollupOptions:{input:{main:fileURLToPath(new URL('./index.html',import.meta.url)),welcome:fileURLToPath(new URL('./welcome.html',import.meta.url)),project:fileURLToPath(new URL('./project.html',import.meta.url)),explorer:fileURLToPath(new URL('./explorer.html',import.meta.url))}} },
};});
