...
export function parseVoiceIntent(value,{pending=null,duration=30,now=new Date()}={}){
  ...
  const transit=numeric(text).match(/^(read )?(?:bus|rts|rider) (?:alerts|updates)(?: for (?:route )?(\d{1,3}[a-z]?))?$/);
  if(transit)return {kind:'command',command:'transit',route:transit[2]?.replace(/^0+(?=\d)/,'').toUpperCase()||null,readAloud:!!transit[1]};
