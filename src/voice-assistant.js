...
export function mountVoiceAssistant({root,getDuration,onConfirm,onCancel=()=>{},getAnswer,speak,stopSpeech,isSpeechSupported,isSpeaking,setProvider=()=>{},host=window}){
  ...
  let provider='browser',configured=false,providerChosen=false,answerVersion=0;
  root.innerHTML=`<div class="voice-intro"><div><p class="eyebrow">YOUR JOURNEY, IN YOUR WORDS</p><h2 id="voice-title">One request at a time.</h2><p>Try “From Reitz Union to Marston tomorrow at eight A M.”</p></div><div class="voice-actions"><button type="button" class="button button-primary" id="voice-talk" aria-describedby="voice-privacy voice-support"><svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10v2a7 7 0 0 0 14 0v-2M12 19v3m-4 0h8"/></svg><span>Talk · microphone off</span></button><button type="button" class="button button-outline" id="voice-stop">Stop audio / microphone</button></div></div>
    <p class="voice-consent">Designed for blind and low-vision people. No map interaction required. Tap Talk to record one request; your selected speech provider processes the audio. No background listening.</p>
    <details class="voice-settings"><summary>Voice settings and privacy</summary><div class="voice-provider-row"><label for="voice-provider">Voice service</label><select id="voice-provider" aria-describedby="voice-privacy voice-support"><option value="browser">Browser voice · fallback</option><option value="elevenlabs" disabled>ElevenLabs · checking connection</option></select></div>
    <p id="voice-privacy" class="voice-small">Microphone access starts only after you tap Talk. ElevenLabs mode records up to 20 seconds in memory, then sends it through RealLens to ElevenLabs for transcription. Reply text is also sent there to create speech. RealLens does not persist recordings or transcripts; ElevenLabs may retain them under its policies. Browser voice uses your browser’s provider, which may also process audio remotely. Stop cancels; no background listening.</p>
    </details><p class="voice-small">AI speech powered by <a href="https://elevenlabs.io" target="_blank" rel="noopener noreferrer">elevenlabs.io</a> · noncommercial prototype. <a href="https://elevenlabs.io/privacy-policy" target="_blank" rel="noopener noreferrer">ElevenLabs privacy policy</a>. Do not share sensitive information.</p>
    <p id="voice-support" class="voice-small"></p><p id="voice-status" class="voice-status" role="status" aria-live="polite" aria-atomic="true">Microphone off. Speak or type a request.</p>
    <div id="voice-conversation" class="voice-conversation" hidden><p class="voice-transcript" id="voice-transcript"></p><h3 id="voice-reply-title">RealLens reply</h3><div id="voice-reply" role="region" aria-labelledby="voice-reply-title" tabindex="0"></div><div class="voice-actions"><button type="button" class="button button-primary" id="voice-confirm" hidden>Confirm trip</button><button type="button" class="button button-outline" id="voice-cancel" hidden>Cancel proposed trip</button><button type="button" class="button button-outline" id="voice-read">Read reply aloud</button></div></div>
    <details class="voice-details"><summary>Type a request or see supported commands</summary><form id="voice-text-form"><label for="voice-text">Trip or briefing question</label><div class="voice-text-row"><input id="voice-text" name="request" type="text" maxlength="1000" autocomplete="off" placeholder="From Reitz to Marston tomorrow at 8 AM"><button class="button button-outline" type="submit">Send request</button></div></form><p class="voice-small">Use Reitz Union, Marston, Smathers, Turlington, the Hub, or Newell. Say today, tomorrow, or a weekday and a time with A M or P M. Duration defaults to your current trip’s estimate (30 minutes if none). Say “for 30 minutes” to set it. Confirm before any journey conditions are requested.</p><div class="voice-chips" role="group" aria-label="Briefing questions"><button type="button" data-question="help">What can I say?</button><button type="button" data-question="rain">Explain rain</button><button type="button" data-question="wind">Wind</button><button type="button" data-question="closures">Closures</button><button type="button" data-question="surroundings">Surroundings</button><button type="button" data-question="unknowns">What’s unknown?</button><button type="button" data-question="briefing">Read briefing</button></div><p class="voice-small">This is a focused command assistant, not open-ended AI or turn-by-turn navigation. Answers use the prepared briefing only. Speech input varies by browser and may require internet. Typed requests and the separate trip form work without a microphone.</p></details>`;
  $('.voice-chips').insertAdjacentHTML('beforeend','<button type="button" data-question="bus alerts">Bus alerts</button>');
  $('.voice-details>.voice-small:last-child').textContent='This is a focused command assistant, not open-ended AI or navigation. Environmental answers use your prepared briefing. “Bus alerts” and “bus alerts for route eleven” use the separate RTS source, even without a walking journey. Speech input varies by browser and may require internet. Typed requests and the separate trip form work without a microphone.';
  ...
  function stop(){answerVersion++;endMicrophone();stopSpeech();audioRequested=false;status('Audio and microphone stopped. Tap Talk for another request.');}
  ...
  async function confirm(audio){
    ...
    show(response?.error||response?.message||'Your briefing is ready.',{audio:audioRequested&&!document.hidden,spoken:response?.speech||response?.error||response?.message});
  }
  function accept(text,audio=false,confidence=1){
    const answerToken=++answerVersion;
    ...
    if(busy&&intent.kind==='command'&&intent.command==='cancel'){version++;pending=null;busy=false;onCancel();stop();updateDraft();show('Voice follow-up cancelled. No further reply will be read. Describe a new trip when ready.');return;}
    if(busy){show('A briefing is being prepared. Wait for it, or type “cancel”.',{audio});return;}
    ...
    if(command==='cancel'){pending=null;version++;updateDraft();show('Proposed trip cancelled. No journey conditions were requested.',{audio});return;}
    ...
    status(command==='transit'?'Checking published RTS information. Microphone off.':'Preparing reply. Microphone off.');
    Promise.resolve().then(()=>getAnswer(command,intent)).then(message=>{
      if(answerToken===answerVersion)show(message,{audio:!document.hidden&&(audio||command==='briefing'||intent.readAloud===true)});
    }).catch(()=>{if(answerToken===answerVersion)show('That information could not be checked. Try again.',{audio:audio&&!document.hidden});});
  }
  function start(){
    providerChosen=true;
