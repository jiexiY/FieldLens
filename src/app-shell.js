...
export function createPageSpeech({onState=()=>{},onError=announce}={}){
  const player=createHybridSpeechPlayer({onState,onError,onNotice:onError});
  ...
  player.destroy=()=>{stop();window.removeEventListener('pagehide',stop);document.removeEventListener('visibilitychange',hidden);document.removeEventListener('keydown',escape);};
  return player;
}
applyPreferences();
