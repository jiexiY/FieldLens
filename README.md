...
Plan submission passes only validated trip inputs through tab-local session storage. A short-lived, one-use confirmation triggers the conditions request on arrival. Reloading or opening Conditions directly requires an explicit check; it does not silently replay the previous request. Session-storage failure is explained, with the standalone typed voice page as fallback. Saved journeys omit the departure time and weather; existing reading preferences remain compatible.
...
Real public **Copernicus Sentinel-2 L2A** data, dated September 22, 2024, is processed using **EMERGE Textbook 1: Data Analysis, Chapter 3, Lesson 3: Vegetation & Water Indices**. The single-scene NDVI adaptation, masks, sampling, missing pixels, and limitations are documented in the source grid, preparation scripts, and /sources. Live NOAA/NWS weather complements this historical context. GLOBE Observer and geoemerge are not used; the track permits other public environmental data. RTS posts alone are not the environmental method.

See [TRACK-COMPLIANCE.md](TRACK-COMPLIANCE.md) for the requirement mapping and [REALLENS-QA.md](REALLENS-QA.md) for current verification. Older HOME-PAGES, HOME-LABELS, DASHBOARD, VOICE, and other QA files describe previous releases and are not evidence that their old selectors match the current pages.
...
Run `npm install`, `npm run dev`, and open http://127.0.0.1:5194/. Vite development/preview middleware and Vercel rewrites resolve all app routes; their `.html` paths also work. Production has eleven HTML entries, including the archive. The middleware provides the same `/api/environment`, `/api/closures`, `/api/transit`, and `/api/voice` endpoints as the Vercel functions. A static file server alone cannot supply live weather, notices, or cloud speech.
...
### Historical voice-assisted pilot (initial browser-only release, September 20, 2026)

Tap **Talk** to request browser microphone access, then say “From Reitz Union to Marston tomorrow at eight A M.” RealLens reads back a proposal and does not change the form or request journey conditions until **Confirm trip** is selected or a separate tapped voice request says “confirm”. Missing places, day, time, and A M/P M produce clarification prompts. A new full route does not inherit an old proposal’s hidden date/time. Voice duration uses the form’s displayed value unless supplied explicitly; the confirmation identifies that default.
